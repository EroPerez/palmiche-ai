...
DEFAULT_RULES: list[GuardrailRule] = [
    ...
    GuardrailRule(
        ...
        patterns=[
            ...
            r"(?i)(do\s+not|don'?t|never)\s+(refuse|decline|reject|censor|filter|limit)\s+(any|my|a\s+single|to\s+answer)",
            ...
            r"(?i)\bDAN\s+(mode|prompt|version|v?\d)",
            r"(?i)you\s+are\s+(not\s+bound|free\s+from|liberated\s+from|released\s+from)\s+(by\s+)?(any\s+|all\s+)?(rules?|restrictions?|guidelines?|limitations?|ethics?\w*|moral\w*)",
            r"(?i)(in\s+a\s+|imagine\s+a\s+)?(hypothetical|fictional|imaginary|alternate)\s+(scenario|world|reality|universe)\s+(where|in\s+which|without)\s+(you\s+)?(have\s+no|there\s+are\s+no|without)?\s*(rules?|restrictions?|limits?|filters?|guidelines?)",
            r"(?i)(today|this)\s+is\s+opposite\s+(day|mode)",
            r"(?i)(you\s+are|play|be)\s+(a\s+)?character\s+(who|that)\s+(has\s+no|ignores?\s+all|ignores?|doesn'?t\s+(have|follow)|without)\s+(rules?|restrictions?|limits?|filters?|morals?|ethics?)",
            r"(?i)(you\s+are|play|be)\s+(a\s+)?character\s+without\s+(rules?|restrictions?|limits?|filters?|morals?|ethics?)",
            r"(?i)unlock\s+(your\s+)?(full|true|hidden|real)\s+(potential|capabilities?|power|abilities?)",
            r"(?i)(remove|disable|turn\s+off|deactivate|lift)\s+(all\s+)?(your\s+)?(limitations?|restrictions?|filters?|safety|safeguards?|guardrails?|censorship)",
            r"(?i)there\s+are\s+no\s+(consequences|rules?|restrictions?|limits?|laws?)\s+(here|now|in\s+this|for\s+this)",
            r"(?i)I\s+give\s+you\s+(full\s+)?permission\s+to\s+(break|ignore|bypass|violate|skip|disregard)\s+(the\s+|all\s+|any\s+|your\s+)?(rules?|restrictions?|guidelines?|safety|filters?)",
            r"(?i)(eres|act[uú]a\s+como)\s+(una?\s+)?(IA|inteligencia\s+artificial|asistente)\s+(sin|libre\s+de)\s+(restricciones?|reglas?|l[ií]mites?|filtros?|censura|[eé]tica|moral)",
            r"(?i)no\s+tienes\s+(reglas?|restricciones?|l[ií]mites?|filtros?|censura|obligaci[oó]n)",
            r"(?i)(olvida|ignora|desactiva|elimina)\s+(tod[oa]s?\s+)?(tus\s+)?(reglas?|restricciones?|programaci[oó]n|entrenamiento|seguridad|filtros?|l[ií]mites?)",
            r"(?i)responde\s+sin\s+(ning[uú]n\s+)?(filtro|restricci[oó]n|censura|l[ií]mite|moral|[eé]tica)",
            r"(?i)(no\s+te\s+niegues|no\s+rechaces|no\s+censures)\s+(a\s+)?(nada|ninguna|ning[uú]n)",
            r"(?i)from\s+now\s+on\s+(ignore|disregard|forget|bypass|skip)\s+(all\s+|any\s+|every\s+)?(rules?|restrictions?|guidelines?|safety|filters?)",
        ],
