...
• Visión por Cámara: capturar fotos desde la cámara, describir escenas, reconocer objetos (con posición y confianza), reconocer gestos de manos (pulgar arriba, paz, puño, señalar, etc.), analizar imágenes con preguntas personalizadas, y monitorear la cámara en tiempo real. Usa el mismo modelo configurado para análisis visual.
...
• Camera Vision: capture photos from the camera, describe scenes, recognize objects (with position and confidence), recognize hand gestures (thumbs up, peace, fist, pointing, etc.), analyze images with custom questions, and monitor the camera in real time. Uses the same configured model for visual analysis.
