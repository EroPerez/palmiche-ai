...
Este archivo (`AGENTS.md`) provee contexto esencial sobre la arquitectura, stack tecnológico y convenciones de desarrollo de **Palmiche JARVIS** para cualquier agente de inteligencia artificial que colabore en el proyecto.
...
- `/jarvis/`: Código fuente del backend en Python (modelos, integraciones, API, utilidades).
  ...
  - `/jarvis/frontend/`: Código fuente del frontend (aplicación web Vue 3).
    - `/jarvis/frontend/src/components/`: Componentes Vue modulares (ej. `ChatMessage.vue`, `TypingIndicator.vue`, `SiriAnimation.vue`).
...
2. **Componentización:** Mantén `App.vue` limpio. Si un bloque de la interfaz (como los mensajes o los indicadores) crece demasiado, extráelo a un componente independiente en `/jarvis/frontend/src/components/`.
