...
- `/www/`: Código fuente del frontend (aplicación web Vue 3).
  - `/www/src/components/`: Componentes Vue modulares (ej. `ChatMessage.vue`, `TypingIndicator.vue`, `SiriAnimation.vue`).
- `run_jarvis.sh` / `run_jarvis.py`: Scripts de inicio del backend.
- `run_web_dev.sh` / `run_web_prod.sh`: Scripts para arrancar el entorno de desarrollo y producción del frontend.
...
2. **Componentización:** Mantén `App.vue` limpio. Si un bloque de la interfaz (como los mensajes o los indicadores) crece demasiado, extráelo a un componente independiente en `/www/src/components/`.
